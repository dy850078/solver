...
```mermaid
sequenceDiagram
    participant GS as Go scheduler
    participant PS as Python solver

    Note over GS: 「4 台 × 8 CPU VM，幫我放」
    GS->>PS: POST /v1/placement/solve
    Note over PS: 嘗試放置 4 台 VM
    PS-->>GS: INFEASIBLE<br/>(anti-affinity 限制只有 3 個 AG)

    Note over GS: 「那改 2 台 × 16 CPU？」<br/>scheduler 不知道最佳解
    GS->>PS: POST /v1/placement/solve
    PS-->>GS: success
```
